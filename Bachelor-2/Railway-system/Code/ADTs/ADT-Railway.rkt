  ...
  (let ((trains '()) ;lijsten die de objecten zullen bijhouden
    ...
    (define (add-train! id previous-seg current-seg)
      (let* ((train (make-train id)))
        ((train 'curr-dtb!) previous-seg)
        ((train 'curr-dtb!) current-seg)
    ...
    (define (train-curr-dtb train-id)
      (let ((train (find-id train-id trains)))
        (train 'curr-dtb)))

    (define (train-curr-dtb! train-id dtb-id)
      (let ((train (find-id train-id trains)))
        ((train 'curr-dtb!) dtb-id)))
    ...
    (define (train-trajectory! train-id new-trajectory)
      (let ((train (find-id train-id trains)))
        ((train 'trajectory!) new-trajectory)))
    ;;
    ...
    (define (dispatch m)
      (cond
        ...
        ((eq? m 'train-curr-dtb) train-curr-dtb)
        ((eq? m 'train-curr-dtb!) train-curr-dtb!)
