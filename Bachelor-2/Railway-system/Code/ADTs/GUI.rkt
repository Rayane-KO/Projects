...
(define (make-gui)
  (let ((window-width 200)
    ...
    (define (start callback) (begin (simulator-button callback) (hardware-button callback) (open-frame main-frame)))
    (define (stop) (close-frame sim/hw-frame))
    ...
    (define (simulator-button callback) (new button%                           
                                             [label "Simulator"]
                                             [parent choice-panel]
                                             [callback (lambda (button event)
                                                         (callback #t)
                                                         (send sim/hw-frame set-label "Railway System - Simulator")
                                                         (open-frame sim/hw-frame)
                                                         (close-frame main-frame))]
                                             [horiz-margin 20]))

    ;voorlopig zal de hardware-button niets doen
    (define (hardware-button callback) (new button%
                                            [label "Hardware"]
                                            [parent choice-panel]
                                            [callback (lambda (button event)
                                                        (callback #f)
                                                        (send sim/hw-frame set-label "Railway System - Hardware")
                                                        (open-frame sim/hw-frame)
                                                        (close-frame main-frame))]
                                            [horiz-margin 20]))

    (define sim/hw-frame (new frame%
                              [label " "]
                              [width sim-width]
                              [height sim-height]
                              [x window-x]
    ...
    (define (print-trajectory trajectory)
      (if (symbol? trajectory)
          (symbol->string trajectory)
          (let loop ((path (reverse trajectory))
                     (res " ]"))
            (if (null? path)
                (begin
                  (set! res (string-append "[" res))
                  res)
                (loop (cdr path) (string-append " " (symbol->string (car path)) res))))))
    ;;
    ;;draw-train
    ;;
      ...
      (let* ((main-panel (new horizontal-panel%     ;*******************;
        ...
        (define start-button
          (new button%
               ...
               [callback (lambda (button event)
                           ...
                           (save-action " started!")
                           ((train-callback 'speed!) max-speed))]))
        ...
        (define inverse-button
          (new button%
               ...
               [callback (lambda (button event) (train-callback 'inverse-speed!) (save-action " changed direction!"))]))
        ...
        (define path-msg
          (new message%
               [parent info-panel]
               [auto-resize #t]
               [label (print-trajectory (train-callback 'trajectory))]))
          ...
          (let ((last-dtb (train-callback 'dt-block))
            ...
            (if (train-callback 'trajectory?)
                (begin
                  ...
                  (disable-controls!))
                (enable-controls!))
            (cond ((not (eq? prev-trajectory trajectory))
                   (send path-msg set-label (print-trajectory (train-callback 'trajectory)))
                   (set! prev-trajectory trajectory)))))
      ...
      (let ((tbl-panel (new horizontal-panel%
        ...
        (define start-button
          (new button%                           
               ...
               [parent tbl-panel]
               [callback (lambda (button event) ((time-tbl-callback 'make-trajectory!) (string-append "trajectories/" (send file-field get-value))) (save-action))]
               [horiz-margin 20]))
        (define (save-action)
          (new message%
               [parent msg-panel]
    ...
    ;;Trajectory
    ;;
    (define trajectory (new horizontal-panel%
                            [parent command-panel]))
    (define choice-t 'none)
    (define (draw-trajectory train-callback dtb-callback)
      (let ((traject-panel (new horizontal-panel%
                                [parent trajectory]
                                [vert-margin 5]
                                [horiz-margin 4]
                                [min-height 100]
        ...
        (set! choice-t
              (new choice%
                   [label "Train: "]
                   [choices (map (lambda (train) (symbol->string (train 'id))) (dtb-callback 'trains))]
                   [horiz-margin 10]
                   [vert-margin 8]
                   [min-width 100]
                   [stretchable-width #f]
                   [parent traject-panel]))
        (define choice-d
          (new choice%
               [label "move to "]
               [choices (map (lambda (dtb) (symbol->string (dtb 'id))) (dtb-callback 'list))]
               [horiz-margin 10]
               [min-width 100]
               [stretchable-width #f]
               [parent traject-panel]))
        (define reset-button
          (new button%                           
               [label "Reset"]
               [parent traject-panel]
               [callback (lambda (button event) ((train-callback (string->symbol (send choice-t get-string-selection))) 'reset-trajectory!))]
               [horiz-margin 20]))
        (define start-button
          (new button%                           
               [label "Start"]
               [parent traject-panel]
               [callback (lambda (button event)
                           (let ((destination (send choice-d get-string-selection)))
                             (((train-callback (string->symbol (send choice-t get-string-selection))) 'make-trajectory!) (string->symbol destination))
                             (save-action destination)))]
               [horiz-margin 20]))
        (define (save-action destination)
          (new message%
               [parent msg-panel]
               [label (string-append (get-time) ": A new automatic trajectory to " destination " was started!")]))
        'ok))
    ;;
    ;;Exit
    ;;
    ...
    (define (dispatch m)
      (cond
        ...
        ((eq? m 'draw-trajectory) draw-trajectory)
